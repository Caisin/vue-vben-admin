...
function createTableActionPermission(
  customPermission?: unknown,
): TableActionPermission {
  if (isFunction(customPermission)) {
    return customPermission as TableActionPermission;
  }

  const { hasAccessByCodes } = useAccess();
  return (auth?: TableActionAuth) => {
    if (!auth) return true;
...
setupVbenVxeTable({
  configVxeTable: (vxeUI) => {
    ...
    // 根据参数库字典编码显示原始 JSON 值对应的标签。
    vxeUI.renderer.add('CellDic', {
    ...
    vxeUI.renderer.add('CellSwitch', {
      renderTableDefault({ attrs, props }, { column, row }) {
        const renderAttrs = { ...props, ...attrs };
        const hasPermission = createTableActionPermission(
          renderAttrs.hasPermission,
        );
        const disabled =
          Boolean(props?.disabled) || !hasPermission(renderAttrs.auth);
        ...
        async function onChange(newVal: any) {
          ...
          row[loadingKey] = true;
          try {
            const result = await renderAttrs.beforeChange?.(newVal, row);
            if (result !== false) {
    ...
    vxeUI.renderer.add('CellOperation', {
      renderTableDefault({ attrs, options, props }, { column, row }) {
        ...
        const operations: Array<Recordable<any>> = (
          ...
          .filter((opt) => opt.show !== false && hasPermission(opt.auth));
        ...
        function renderConfirm(opt: Recordable<any>) {
          ...
          return h(
            Popconfirm,
            {
              ...
              placement: 'topLeft',
              title: $t('ui.actionTitle.delete', [renderAttrs.nameTitle || '']),
              ...props,
              ...opt,
              icon: undefined,
              ...
              description: () =>
                h(
                  ...
                  $t('ui.actionMessage.deleteConfirm', [
                    row[renderAttrs.nameField || 'name'],
                  ]),
                ),
            },
          );
...
export const VbenTableAction = defineComponent(
  (props: TableActionProps, { attrs, slots }) => {
    const hasPermission = createTableActionPermission(
      props.hasPermission ?? attrs.hasPermission,
    );
    return () =>
      h(VbenTableActionCore, { ...props, ...attrs, hasPermission }, slots);
  },
  {
