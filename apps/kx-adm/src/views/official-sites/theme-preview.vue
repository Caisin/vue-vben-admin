...
const theme = computed(
  () => themes.find((item) => item.value === route.query.theme) ?? themes[0],
);
const layout = computed(
  () => layouts.find((item) => item.value === route.query.layout) ?? layouts[0],
);
</script>
<template>
  <main
  ...
  >
    <header class="mx-auto max-w-6xl">
      ...
      <h1 class="mt-8 text-7xl font-bold">{{ theme.sample }}</h1>
      <p class="mt-4 text-xl">{{ layout.label }} · {{ layout.detail }}</p>
    </header>
    <section class="mx-auto mt-16 grid max-w-6xl grid-cols-3 gap-5">
      <article
