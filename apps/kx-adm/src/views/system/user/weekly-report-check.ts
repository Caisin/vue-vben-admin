...
export async function waitForWeeklyReportCheck(
  ...
  const publish = await waitForTask({
    isCurrent,
    load: () => SystemUserApi.weekly_report_publish(publishId, task.id),
    done: (publish) => {
    ...
    timeoutMessage: '周报检查仍在执行，请稍后刷新名单',
  });
  if (!publish) return;
  ...
  if (!isCurrent()) return;
  return {
    participants: participants.filter((item) => item.status !== 'completed'),
    publish,
  };
}
