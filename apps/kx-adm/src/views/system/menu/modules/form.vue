...
type MenuSubmitValues = Omit<SystemMenu, 'children' | 'id'>;
...
function hasMenuType(type: unknown, values: string[]) {
...
const schema: VbenFormSchema<MenuFormValues>[] = [
  ...
  },
  {
    ...
    rules: z
      ...
      .refine(
        async (value: string) => {
          return !(await SystemMenuApi.nameExists(value, formData.value?.id));
        },
        {
          error: (issue) =>
  ...
  },
  {
    ...
    componentProps: {
      api: SystemMenuApi.list,
      afterFetch: (items: SystemMenu[]) =>
        wrapTreeWithRoot(items, {
          authCode: '',
          id: '0',
          meta: { title: '根菜单' },
          name: 'RootMenu',
  ...
  },
  {
    ...
    dependencies: {
      show: (values) => {
        return hasMenuType(values.type, ['catalog', 'embedded', 'menu']);
      },
      triggerFields: ['type'],
    },
    fieldName: 'path',
    ...
    rules: z
      ...
      .refine(
        async (value: string) => {
          return !(await SystemMenuApi.pathExists(
            value || '',
            formData.value?.id,
          ));
        },
  ...
  },
  {
    ...
    dependencies: {
      show: (values) => {
        return hasMenuType(values.type, ['embedded', 'menu']);
      },
      triggerFields: ['type'],
    },
    ...
    rules: z
      ...
      .refine(async (value: string) => {
        return await SystemMenuApi.pathExists(value || '', formData.value?.id);
  ...
  },
  {
    ...
    dependencies: {
      show: (values) => {
        return hasMenuType(values.type, [
          'catalog',
          'embedded',
          'link',
          'menu',
  ...
  },
  {
    ...
    dependencies: {
      show: (values) => {
        return hasMenuType(values.type, ['embedded', 'link']);
      },
      triggerFields: ['type'],
    },
    fieldName: 'linkSrc',
  ...
  },
  {
    ...
    dependencies: {
      show: (values) => {
        return !hasMenuType(values.type, ['button', 'link']);
      },
      triggerFields: ['type'],
    },
    fieldName: 'divider1',
  ...
  },
  {
    ...
    dependencies: {
      show: (values) => {
        return hasMenuType(values.type, ['menu']);
      },
      triggerFields: ['type'],
    },
  ...
  },
  {
    ...
    dependencies: {
      show: (values) => {
        return !hasMenuType(values.type, ['button']);
      },
      triggerFields: ['type'],
    },
  ...
  },
  {
    ...
    dependencies: {
      show: (values) => {
        return hasMenuType(values.type, ['catalog', 'menu']);
      },
      triggerFields: ['type'],
    },
  ...
  if (valid) {
    ...
    try {
      await (formData.value?.id
        ? SystemMenuApi.update(formData.value.id, data)
        : SystemMenuApi.create(data));
      drawerApi.close();
      emit('success');
    } finally {
      drawerApi.unlock();
