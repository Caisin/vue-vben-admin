...
import FamilyForm from './modules/form.vue';

const userOptionList = ref<ReturnType<typeof userOptions>>([]);
const userLabelMap = ref(new Map<string, string>());

const [FormDrawer, formDrawerApi] = useVbenDrawer({
  connectedComponent: FamilyForm,
  destroyOnClose: true,
});

const [Grid, gridApi] = useVbenVxeGrid<WmxtFamily>({
  ...
  } as VxeTableGridOptions<WmxtFamily>,
});
...
function openEdit(row: WmxtFamily) {
...
async function onRemove(row: WmxtFamily) {
  if (row.id === undefined) return;
  await WmxtAdminApi.remove_family(row.id);
  onRefresh();
}
</script>
...
<template>
  ...
  >
    ...
    <Grid class="management-grid" table-title="家庭管理">
      <template #toolbar-tools>
        <Button
          v-access:code="'wmxt:family:write'"
          type="primary"
          @click="openCreate"
        >
          <Plus class="size-5" />新建家庭
        </Button>
      </template>
