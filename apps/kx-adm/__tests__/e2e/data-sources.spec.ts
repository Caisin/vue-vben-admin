...
for (const width of [1280, 360]) {
  test(`数据源 ${width}px 可首次新增、再次编辑并测试 Databend 连接`, async ({
    page,
  }, testInfo) => {
    ...
    await expect(page.locator('.vxe-table--fixed-right-wrapper')).toHaveCount(
      0,
    );
    await page.getByRole('button', { name: '新增数据源' }).click();
    ...
    await expect(page.locator('.vxe-table--fixed-right-wrapper')).toBeVisible();
    await expect(
      page.getByText('分析仓库已修改', { exact: true }),
    ).toBeVisible();
  });
}
